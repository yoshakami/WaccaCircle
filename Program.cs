                    ...
                    for (int j = 0; j < 60; j++)
                    {
                        if (controller.touchData[i, j])  // if the circle if touched
                        {
                            ...
                            if (i > 1)  // RXY is only on the two outer layers, i==2 and i==3
                            {
                                ...
                                {
                                    button_pressed_on_loop[axes[j][2] + 48] = true;
                                    if (!button_pressed[axes[j][2] + 48])
                                    {
                                        joystick.SetBtn(true, deviceId, (uint)(axes[j][2] + 48)); // Press button axes[j][2] + 24
                                        button_pressed[axes[j][2] + 48] = true;
                                    }

                                    button_pressed_on_loop[axes[j][2] + 24] = true;
                                    if (!button_pressed[axes[j][2] + 24])
                                    {
                                        joystick.SetBtn(true, deviceId, (uint)(axes[j][2] + 24)); // Press button axes[j][2] + 24
                                        button_pressed[axes[j][2] + 24] = true;
                                    }
                                }
                                rx_pressed = true;
